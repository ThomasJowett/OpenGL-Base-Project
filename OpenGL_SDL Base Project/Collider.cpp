...
bool Sphere::CheckCollision(Collider * otherCollider, Vector3D & normal, float & penetrationDepth)
{
	mCollided = false;
	//Sphere on Sphere
	if (Sphere* otherSphere = dynamic_cast<Sphere*>(otherCollider))
	{
		...
		float sumOfBoundingRadii = mRadius + otherSphere->GetBoundingRadius();
...
bool AABB::CheckCollision(Collider * otherCollider, Vector3D & normal, float & penetrationDepth)
{
	mCollided = false;
	//Box on Sphere
	if (Sphere* otherSphere = dynamic_cast<Sphere*>(otherCollider))
