...
IndexedModel OBJModel::ToIndexedModel()
{
	...
	for (unsigned int i = 0; i < numIndices; i++)
	{
		...
		Vector3D currentPosition = vertices[currentIndex->vertexIndex];
		...
		if (hasUVs)
			...
			currentTexCoord = Vector2D(0, 0);

		if (hasNormals)
			currentNormal = normals[currentIndex->normalIndex];
		else
			currentNormal = Vector3D(0, 0, 0);
...
Vector3D OBJModel::ParseOBJVec3(const std::string& line)
{
...
Vector2D OBJModel::ParseOBJVec2(const std::string& line)
{
	unsigned int tokenLength = line.length();
	const char* tokenString = line.c_str();
